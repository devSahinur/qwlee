...
const BuyerOrderInfo = () => {
  const { data: activeOrders, isFetching } = useGetBuyerAllOrderQuery("active");
  const { data: missingDetailsOrder } = useGetBuyerAllOrderQuery("late");
  const { data: deliveredOrders } = useGetBuyerAllOrderQuery("delivered");
  const { data: cancelledOrders } = useGetBuyerAllOrderQuery("cancelled");
  ...
  const mapOrderData = (orders) => {
    return orders?.map((item) => {
      ...
      return {
        key: item._id,
        image:
          item?.gigId?.images[0] && imgUrl(item?.gigId?.images[0]),
        freelancerName: item?.freelancerId?.fullName,
